...
export default function StressRadar({ score, isActive }: StressRadarProps) {
  ...
  return (
    <div className="w-full bg-slate-900 rounded-2xl p-6 border border-slate-800 shadow-2xl relative overflow-hidden">
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="flex items-center gap-2 px-2 py-1 bg-slate-950 rounded border border-slate-800">
          {isActive && <span className="w-1.5 h-1.5 rounded-full bg-rose-500 animate-pulse"></span>}
          <span className="text-[10px] text-slate-400 font-mono tracking-widest uppercase">Live_Analysis</span>
        </div>
      </div>

      <div className="flex items-end gap-4 mb-8 z-10 relative">
        <div className="text-5xl font-mono text-white tracking-tighter">
          {Math.round(score)}
        </div>
        <div className="pb-1">
          <div className={`text-xs font-mono tracking-widest uppercase mb-1 ${textColor}`}>
            {status}
          </div>
          <div className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Composite_Score // 100</div>
        </div>
      </div>

      {/* Visual Bar */}
      <div className="relative h-4 bg-slate-950 rounded-full overflow-hidden border border-slate-800/50">
        {/* Markers */}
